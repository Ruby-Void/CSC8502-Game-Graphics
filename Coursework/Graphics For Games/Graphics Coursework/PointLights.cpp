...
PointLights::PointLights(int lightNum, int rawWidth, int rawHeight,
	...
	pointlightShader = new Shader(VERTEXDIR"PointLightVertex.glsl", FRAGMENTDIR"PointLightFragment.glsl");
	if (!pointlightShader->LinkProgram()) {
		pointLightsInit = false;
	}
	else {
		...
		for (int x = 0; x < lightNum; ++x) {
			for (int z = 0; z < lightNum; ++z) {
				...
				float r = 0.5f + (float)(rand() % 129) / 128.0f, g = 0.5f + (float)(rand() % 129) / 128.0f, b = 0.5f + (float)(rand() % 129) / 128.0f;
