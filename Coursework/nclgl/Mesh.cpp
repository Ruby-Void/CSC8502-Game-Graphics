...
Mesh::Mesh(void) : 
	numVertices(0), texture(0), numIndices(0), 
	bumpTexture(0), indices(nullptr), vertices(nullptr), 
	normals(nullptr), tangents(nullptr), colours(nullptr), 
	textureCoords(nullptr), type(GL_TRIANGLES) {
...
void Mesh::GenerateNormals() {
	...
	if (indices) { // Generate per - vertex normals
		...
		for (GLuint i = 0; i < numVertices; i += 3) {
			...
			normals[i] = normals[i + 1] = normals[i + 2] = Vector3::Cross(b - a, c - a);
		}
	}
...
Vector3 Mesh::GenerateTangent(const Vector3& a, const Vector3& b, const Vector3& c, const Vector2& ta, const Vector2& tb, const Vector2& tc) {
	Vector2 coord1 = tb - ta, coord2 = tc - ta;
	Vector3 vertex1 = b - a, vertex2 = c - a;
