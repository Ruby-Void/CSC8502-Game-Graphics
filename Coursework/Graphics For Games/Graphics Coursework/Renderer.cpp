...
Renderer::Renderer(Window& parent) : OGLRenderer(parent), waterRotation(0.0f), lightRotation(0.0f) {
	...
	if (!skybox->getSkyboxInit() || !pointLights->getPointLightsInit() || createHeightMap() || 
	...
	envIllum = new Light(Vector3(-1000.0f, 1000.0f, -1000.0f), Vector4(0.9f, 0.9f, 1.0f, 1), 100000.0f);
	root = new SceneNode();
...
bool Renderer::createWater() {
	...
	waterIllum = new Light(Vector3((RAW_HEIGHT * HEIGHTMAP_X / 2.0f), 500.0f, (RAW_HEIGHT * HEIGHTMAP_Z / 2.0f)), Vector4(0.9f, 0.9f, 1.0f, 1), (RAW_WIDTH * HEIGHTMAP_X));
...
bool Renderer::createShaders() {
	...
	combineShadowShader = new Shader(SHADERDIR"ShadowShaders/shadowSceneVertex.glsl", SHADERDIR"ShadowShaders/shadowSceneFragment.glsl");
	shadowShader = new Shader(SHADERDIR"ShadowShaders/shadowVertex.glsl", SHADERDIR"ShadowShaders/shadowFragment.glsl");
	waterShader = new Shader(SHADERDIR"PerPixelShaders/PerPixelVertex.glsl", SHADERDIR"ReflectFragment.glsl");
	combinedShader = new Shader(SHADERDIR"CombineShaders/AdvanceCombineVertex.glsl", SHADERDIR"CombineShaders/AdvanceCombineFragment.glsl");
	return (sceneShader->LinkProgram() && combineShadowShader->LinkProgram() && shadowShader->LinkProgram() && waterShader->LinkProgram() && combinedShader->LinkProgram()) ? false : true;
}
...
bool Renderer::createFBOs() {
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, testST, 0);
	glDrawBuffer(GL_COLOR_ATTACHMENT0);
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) { return true; }
...
void Renderer::bindEnvBuffer() {
	...
	glBindTexture(GL_TEXTURE_CUBE_MAP, skybox->getSkyboxCubeMap());
...
void Renderer::bindPointLightsBuffer() { // Alter this to be more complexe (e.g. different sized lights, colour changing)
	glBindFramebuffer(GL_FRAMEBUFFER, pointLightFBO);
	glClear(GL_COLOR_BUFFER_BIT);
...
void Renderer::bindShadowBuffer() {
	...
	viewMatrix = Matrix4::BuildViewMatrix(envIllum->GetPosition(), Vector3(4000.0f, 0.0f, 4000.0f));
	UpdateShaderMatrices();
	drawNodes();
...
void Renderer::combineShadowBuffer() {
	glBindFramebuffer(GL_FRAMEBUFFER, testFBO);
	SetCurrentShader(combineShadowShader);
